 ...
 */
public class ProxyListenerALMEx implements IProxyListenerALM
{
	private static final int COID_SHOWDEFAULTICON = 64200;

	private boolean mHMIStarted = false;
	private Integer mIconID = null;
	...
	public void setParam(SdlProxyEx proxy, IProxyListenerALMEx forwardListener,
						 Integer iconID, FileType iconFileType)
	{
		mProxy = proxy;
	...
	{
		if (!mHMIStarted && mIconID != null && mIconFileType != null)
		{
			mHMIStarted = true;

			try
			{
				mProxy.showAppIcon(mIconID, mIconFileType, COID_SHOWDEFAULTICON);
			}
...
//		mProxy.reset();
	...
	{
		if (mForwardListener != null)
			mForwardListener.onButtonPressResponse(response);
	}
	...
	{
		if (mForwardListener != null)
			mForwardListener.onGetInteriorVehicleDataCapabilitiesResponse(response);
	}
	...
	{
		if (mForwardListener != null)
			mForwardListener.onGetInteriorVehicleDataResponse(response);
	}
	...
	{
		if (mForwardListener != null)
			mForwardListener.onSetInteriorVehicleDataResponse(response);
	}
	...
	{
		if (mForwardListener != null)
			mForwardListener.onOnInteriorVehicleData(notification);
	}
	...
	{
		if (mForwardListener != null)
			mForwardListener.onServiceDataACK();
	}
